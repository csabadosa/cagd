...
GLboolean GenericCurve3::UpdateVertexBufferObjects(GLdouble scale,GLenum usage_flag)
{
    ...
    for (GLuint i = 0; i < curve_point_count; ++i)
    {
        ...
        {
            *coordinate = static_cast<GLfloat>(_derivative(0,i)[j]);
            ++coordinate;
        }
    }
    ...
    for (GLuint d = 1; d < _derivative.GetRowCount(); ++d)
    {
        ...
        glBufferData(GL_ARRAY_BUFFER, higher_order_derivative_byte_size, nullptr, _usage_flag);
        ...
        for (GLuint i = 0; i < curve_point_count; ++i)
        {
            ...
            {
                *coordinate = static_cast<GLfloat>(_derivative(0, i)[j]);
                *(coordinate + 3) = static_cast<GLfloat>(sum[j]);
                ++coordinate;
            }
