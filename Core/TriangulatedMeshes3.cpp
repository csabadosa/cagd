...
GLboolean TriangulatedMesh3::SaveToOFF(const std::string& file_name) const
{
    ...
    f << "OFF" << endl;
    f << _vertex.size() <<endl << _face.size() << endl << 0 << endl;
    ...
    for(vector<TriangularFace>::const_iterator it = _face.begin();it != _face.end();++it)
    {
        f << *it << " ";
    }
...
{
    lhs << rhs.VertexCount() << rhs.FaceCount() << endl;
    lhs << rhs._leftmost_vertex << rhs._rightmost_vertex << endl;
    for(auto &r : rhs._vertex)
        lhs << r << endl;
    for(auto &r : rhs._normal)
        lhs << r <<endl;
    for(auto &r : rhs._tex)
        lhs << r <<endl;
    for(auto &r : rhs._face)
        lhs << r << endl;
    return lhs;
}
